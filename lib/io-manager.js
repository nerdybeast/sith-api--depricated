...
module.exports = function(io) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('initialize-dashboard', function(profile, callback) {
            
            let socketId = this.id;

            _debug.log(`initialize-dashboard received user id => ${profile.user_id}`);
            ...
            let run = () => {
                ...
                if(!orgPollers.has(profile.username)) { return; }
            ...
            if(!orgPollers.has(profile.username)) {
                orgPollers.add(profile.username);
                run();
            }
