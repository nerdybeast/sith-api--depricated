...
//NOTE: Basically this will append every redis function with the word "Async" so .get('key', callback) becomes .get('key').then(...)
...
module.exports.set = function(key, val, ttl) {
    ...
    if(ttl && typeof ttl !== 'number') {
        let exception = new Error(`ttl supllied, expected number but received ${typeof ttl}`);
        return Promise.reject(exception);
    }
    ...
    return client.setAsync(key, val).then(cacheSetResult => {

        _debug.log(`Result of caching the key ${key}`, cacheSetResult);

        let props = { cacheSetResult };

        if(ttl) {
            props.expire = client.expire(key, ttl);
        }
        ...
        if(result.expire) {
            _debug.log(`Result of setting an expiration of ${ttl} seconds on key ${key}`, result.expire);
        }

        _debug.log(`Successfully store cache key ${key}`);
        return result.cacheSetResult;
