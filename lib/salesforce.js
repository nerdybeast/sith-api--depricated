...
class Salesforce {
    ...
    getAllDebugLevels(bypassCache) {

        const cacheKey = `DEBUG_LEVELS:${this._user.orgId}`;
    ...
    globalDescribe() {
        ...
        return this._standardGlobalDescribe().then(data => {

            let fieldNames = _.keys(data.sobjects[0]);

            //When describing these sobjects, they don't have an id so let's give them one here.
            data.sobjects.forEach(sobject => sobject.id = sobject.name);

            return { fieldNames, sobjects: data.sobjects };
        });
    }

    describeSobject(sobjectName) {

        let cacheKey = `SOBJECT_DESCRIBE_BY_NAME:${this._user.orgId}:${sobjectName}`;

        //return cache.get(cacheKey).then(cachedResult => {
            //if(cachedResult) { return cachedResult; }
            return this.conn.describe(sobjectName).then(result => {
                let fieldNames = _.keys(result);
    ...
    _fullDescribe() {
        ...
        const cacheKey = `GLOBAL_SOBJECT_NAMES_DESCRIBE_BY_ORG:${this._user.orgId}`;
    ...
    _standardGlobalDescribe() {

        let cacheKey = `GLOBAL_SOBJECT_DESCRIBE_BY_ORG:${this._user.orgId}`;
        ...
        return cache.get(cacheKey).then(cachedDescribe => {
            ...
            return this.conn.describeGlobal().then(data => {
                
                //cache for 12 hours
                let cachePromise = cache.set(cacheKey, data, (60 * 60 * 12));
                
                return Promise.props({ data, cachePromise});
    ...
    _restoreTraceFlags(userId) {
        ...
        this.conn.tooling.sobject('TraceFlag').del([user.analyticsTraceFlagId]).then(result => {
            ...
            this.io.emit('sobject-update', {
                type: 'TraceFlag',
                action: 'delete',
                id: user.analyticsTraceFlagId
            });
