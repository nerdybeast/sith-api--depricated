...
jsforceExt.prototype.watchOrgLimits = function(orgId, isActive) {

    // const cacheKey = `WATCH_ORG_LIMIT:${orgId}`;
    // let cachedOrgLimitWatcher = cache.get(cacheKey);
    
    // //Will be true if the current org is already being polled for changes.
    // if(cachedOrgLimitWatcher !== undefined) {
    //     return;
    // }

    // cache.set(cacheKey, true);

    // let socket = this.io.of(`/org-limit-change:${orgId}`);
    ...
    //     _debug.log(`Client Connected => ${socket.id}`);
        ...
        let run = () => {
            ...
            this.getOrgLimits().then(currentLimits => {
                
                if(!_.isEqual(limits, currentLimits)) {
                    limits = currentLimits;
                    socket.emit('org-limits-update', limits);
                }

                runTimer = setTimeout(() => run(), 5000);
