...
jsforceExt.prototype.getTraceFlagsByUserId = function(userId) {
    ...
    return Promise.all([
        ...
        this.getSobjectFieldNames('DebugLevel')
    ]).then(result => {

        let traceFlagFieldNames = result[0];
        let debugLevelFieldNames = result[1];

        debugLevelFieldNames.forEach(field => traceFlagFieldNames.push(`DebugLevel.${field}`));
...
jsforceExt.prototype.getAllDebugLevels = function(bypassCache) {
    ...
    return cache.get(cacheKey, bypassCache).then(cachedDebugLevels => {

        if(cachedDebugLevels) {
            return cachedDebugLevels;
        }

        return this.getSobjectFieldNames('DebugLevel').then(debugLevelFieldNames => {

            let soql = `Select ${debugLevelFieldNames.join(',')} From DebugLevel`;
            return this.conn.tooling.query(soql);

        }).then(queryResult => {
            
            return Promise.props({
                queryResult,
                cacheResult: cache.set(cacheKey, queryResult, (60 * 60 * 12)) //cache for 12 hours
            });

        }).then(hash => {
            return hash.queryResult;
        });
