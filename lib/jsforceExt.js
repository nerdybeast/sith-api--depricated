...
jsforceExt.prototype.triggerAsyncTestRun = function(testClassIds) {
    ...
    return Promise.all([
        ...
        _debug(`${apexTestQueueItemType} field names => ${apexTestQueueItemFieldNames.join(', ')}`);
...
jsforceExt.prototype.getTestRunStatus = function(asyncApexJobId) {
    ...
    let run = (asyncApexJobId) => {
        ...
        this.conn.query(`SELECT ${apexTestQueueItemFieldNames.join(',')} FROM ${sobjectType} WHERE ParentJobId = '${asyncApexJobId}'`).then((res) => {
            ...
            //Will be true on the first poll loop meaning that the queue hasn't been instantiated yet.
            //if(!pollingQueue) { pollingQueue = records; }
            ...
            //Now that the changed records have been captured, reset the queue to the current records state so that the next poll will capture additional chnages.
            ...
            if(outputMessage !== tempMessage) {
                ...
                //     //TODO: This is going to process a completed test more than once, need to fix.
...
jsforceExt.prototype._processTestResult = function(apexTestQueueItemIds, asyncApexJobId) {
    ...
    Promise.all([
        ...
        //_debug(`Example ${apexTestResultType} record`, res.records[0]);
        
        let apexTestResultData = new serializer(apexTestResultType, { 
            attributes: _.without(apexTestResultFieldNames, 'id')
        }).serialize(apexTestResultRecords);
        
        let asyncApexJobData = new serializer(asyncApexJobType, { 
            attributes: _.without(asyncApexJobFieldNames, 'id')  
        }).serialize(asyncApexJobRecords);
        ...
        //NOTE: res here will be an array of debug logs.
        
        res.forEach((log) => {
            let analytics = extractAnalytics(log);
            _debug('Analytics found', analytics.length);
        });
