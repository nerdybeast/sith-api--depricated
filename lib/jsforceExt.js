...
jsforceExt.prototype.getTestRunStatus = function(asyncApexJobId) {
    ...
    let run = (asyncApexJobId) => {
        ...
        this.conn.query(`SELECT ${apexTestQueueItemFieldNames.join(',')} FROM ${sobjectType} WHERE ParentJobId = '${asyncApexJobId}'`).then((res) => {
            ...
            let emitQueue = _getModifiedRecords(records, queue);
            ...
            if(emitQueue.length > 0) { 
                this.io.emit('test-status', { sobjectType, records: emitQueue, isCompleted });
            }
...
jsforceExt.prototype._processTestResult = function(apexTestQueueItemIds) {
    ...
    this.getSobjectFieldNames(sobjectType).then((fieldNames) => {
        ...
        apexTestResultFieldNames = fieldNames;
        return this.conn.query(`SELECT ${fieldNames.join(',')} FROM ${sobjectType} WHERE QueueItemId IN ('${apexTestQueueItemIds.join("','")}')`);
         
    }).then((res) => {
        
        let apexTestResultRecords = _camelizeObjectArray(res.records);
        
        if(res.totalSize > 0) {
            _debug(`Example ${sobjectType} record`, res.records[0]);
        }
        
        this.io.emit('records-update', customSerializer.serializeForUpdate('apex-test-result', apexTestResultFieldNames, apexTestResultRecords));
